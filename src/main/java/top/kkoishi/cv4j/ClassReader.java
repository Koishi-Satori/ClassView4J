...
import top.kkoishi.cv4j.attr.DeprecatedAttribute;
...
import top.kkoishi.cv4j.attr.RuntimeInvisibleParameterAnnotationsAttribute;
import top.kkoishi.cv4j.attr.RuntimeTypeAnnotationAttribute;
import top.kkoishi.cv4j.attr.RuntimeVisibleParameterAnnotationsAttribute;
...
import top.kkoishi.cv4j.attr.frames.verifi.DoubleVariableInfo;
import top.kkoishi.cv4j.attr.frames.verifi.FloatVariableInfo;
import top.kkoishi.cv4j.attr.frames.verifi.IntegerVariableInfo;
import top.kkoishi.cv4j.attr.frames.verifi.LongVariableInfo;
import top.kkoishi.cv4j.attr.frames.verifi.NullVariableInfo;
import top.kkoishi.cv4j.attr.frames.verifi.ObjectVariableInfo;
import top.kkoishi.cv4j.attr.frames.verifi.TopVariableInfo;
import top.kkoishi.cv4j.attr.frames.verifi.UninitializedVariableInfo;
import top.kkoishi.cv4j.attr.frames.verifi.UninitialzedThis;
import top.kkoishi.cv4j.attr.frames.verifi.VerificationTypeInfo;
import top.kkoishi.cv4j.cp.ConstClassInfo;
import top.kkoishi.cv4j.cp.ConstDoubleInfo;
import top.kkoishi.cv4j.cp.ConstDynamicInfo;
import top.kkoishi.cv4j.cp.ConstFieldrefInfo;
import top.kkoishi.cv4j.cp.ConstFloatInfo;
import top.kkoishi.cv4j.cp.ConstIntegerInfo;
import top.kkoishi.cv4j.cp.ConstInterfaceMethodrefInfo;
import top.kkoishi.cv4j.cp.ConstInvokeDynamicInfo;
import top.kkoishi.cv4j.cp.ConstLongInfo;
import top.kkoishi.cv4j.cp.ConstMethodHandleInfo;
import top.kkoishi.cv4j.cp.ConstMethodTypeInfo;
import top.kkoishi.cv4j.cp.ConstMethodrefInfo;
import top.kkoishi.cv4j.cp.ConstModuleInfo;
import top.kkoishi.cv4j.cp.ConstNameAndTypeInfo;
import top.kkoishi.cv4j.cp.ConstPackageInfo;
...
public class ClassReader implements Closeable {
     ...
     *
     * @see top.kkoishi.cv4j.attr.RuntimeAnnotationAttribute.Annotation.ElementValuePairs.ElementValue.Value
     */
    ...
    public ArrayList<Attribute_info> getClassFileAttributeTable () {
    ...
    public static void report (ClassReader cr) {
        ...
        for (final FieldInfo fieldInfo : cr.getFieldTable()) {
            ...
            if (fieldInfo.getAttributeCount() == 0) {
                ...
                final ArrayList<Attribute_info> attributes = fieldInfo.getAttributes();
                for (int i = 0; i < attributes.size() - 1; i++) {
                    final Attribute_info attribute = attributes.get(i);
    ...
    protected Attribute_info[] readMethodAttributeTable (int count) throws IllegalAttributeNameException, IllegalRuntimeAnnotationAttributeElementValueTagByteException {
        if (count == 0) {
            return new Attribute_info[0];
        }
        final Attribute_info[] attributes = new Attribute_info[count];
    ...
    protected Attribute_info[] readCodeAttributeTable (int count) throws IllegalAttributeNameException {
        if (count == 0) {
            return new Attribute_info[0];
        }
        final Attribute_info[] attributes = new Attribute_info[count];
    ...
    private void addRecordImpl (int attributeNameIndex, int attributesLength, String name)
        ...
        for (int j = 0; j < componentsCount; j++) {
            ...
            final Attribute_info[] attributes = new Attribute_info[attributesCount];
    ...
    protected ArrayList<Attribute_info> readFieldAttributeTable (int count)
            throws IllegalAttributeNameException, IllegalRuntimeAnnotationAttributeElementValueTagByteException {
        final var data = new ArrayList<Attribute_info>(count);
        for (int i = 0; i < count; i++) {
            final int nameIndex = toInt(br.read(2));
            final int attributeLength = toInt(br.read(4));
